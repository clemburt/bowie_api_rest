"""
Test suite for the main application API endpoints.

Contains tests for searching albums, tracks, and related functionalities.
"""

from fastapi.testclient import TestClient
import pytest

from bowie_api_rest.main import app


@pytest.fixture(scope="module")
def client():
    """
    Set up and tear down the FastAPI test client.

    Provide a reusable client instance for testing.
    """
...
def test_search_tracks_in_albums(client: TestClient):
    ...
    Check that the track is included in the returned album data.
    """
    ...
    found_album = next((album for album in albums if album["title"] == "David Bowie"), None)
...
def test_search_tracks_case_insensitive(client: TestClient):
    ...
    Check that track titles are matched regardless of case.
    """
...
def test_search_tracks_partial_title(client: TestClient):
    ...
    Check if albums containing tracks with partial titles are returned.
    """
    response = client.get("/tracks/Space/albums")  # Searching with a partial track title
    assert response.status_code == 200
    albums = response.json()

    # Check that at least one album with a track containing "Space" is returned
    assert len(albums) > 0
    found_album = next((album for album in albums if "David Bowie" in album["title"]), None)
    assert found_album is not None
    track_titles = [track["title"] for track in found_album["tracks"]]
...
def test_search_albums_by_title(client: TestClient):
    ...
    Check if albums with a matching partial title are returned.
    """
    response = client.get("/albums/by-title/?album_title=Hunky")  # Partial album title search
    assert response.status_code == 200
    albums = response.json()

    # Assert that the album "Hunky Dory" is returned
    assert len(albums) > 0
    found_album = next((album for album in albums if album["title"] == "Hunky Dory"), None)
    assert found_album is not None
...
def test_search_albums_case_insensitive(client: TestClient):
    ...
    Check if the search is case-insensitive and still matches albums.
    """
    response = client.get("/albums/by-title/?album_title=hunky dory")  # Case-insensitive search
    assert response.status_code == 200
    albums = response.json()
...
def test_search_album_by_exact_title(client: TestClient):
    ...
    Check that only albums with an exact match for the title are returned.
    """
    response = client.get("/albums/by-title/?album_title=Diamond Dogs")  # Exact album title search
    assert response.status_code == 200
    albums = response.json()
...
def test_search_multiple_tracks(client: TestClient):
    ...
    Check if multiple tracks exist within the album.
    """
    response = client.get("/tracks/Young/albums")  # Searching for multiple tracks with partial title
    assert response.status_code == 200
    albums = response.json()

    assert len(albums) > 0
    found_album = next((album for album in albums if album["title"] == "Young Americans"), None)
    assert found_album is not None
    track_titles = [track["title"] for track in found_album["tracks"]]
...
def test_search_album_by_year(client: TestClient):
    ...
    Check if albums are correctly filtered by the release year.
    """
    response = client.get("/albums/by-title/?album_title=The Man Who Sold the World")  # Album with exact title
    assert response.status_code == 200
    albums = response.json()
...
def test_search_album_with_track_duration(client: TestClient):
    ...
    Check that the duration of a track matches the expected value.
    """
    ...
    found_album = next((album for album in albums if album["title"] == "Let's Dance"), None)
    assert found_album is not None
    track = next(
...
def test_search_for_non_existent_track(client: TestClient):
    ...
    Check that the API handles searches with no results correctly.
    """
