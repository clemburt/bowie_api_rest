...
from bowie_api_rest.main import app  # Assuming FastAPI app is in the main module
...
def test_search_album_by_year(client: TestClient):
    ...
    found_album = next((album for album in albums if album["title"] == "The Man Who Sold the World"), None)
    assert found_album is not None
...
def test_search_album_with_track_duration(client: TestClient):
    ...
    assert found_album is not None
    track = next((track for track in found_album["tracks"] if track["title"] == "Modern Love"), None)
